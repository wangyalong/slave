...
csvf = file('../csvSet/online_name_20160422.csv', 'r')
...
for element in data:
    ...
    id2 = element[2]
    station = element[3]
    onlineName = element[4].lower()
    id1, id2, station, onlineName
    if id1 != '':
        sql1 = 'update station set online_name = "%s" where station_id = "%s" and station = "%s"' % (onlineName, id1, station)
        ExecuteSQL(sql1)
        print sql1
    else:
        pass
    sql = 'update station set online_name = "%s" where station_id = "%s" and station = "%s"' % (onlineName, id2, station)
    print sql
    ExecuteSQL(sql)
