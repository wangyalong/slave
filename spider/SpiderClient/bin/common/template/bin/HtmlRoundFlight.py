...
class HtmlRoundFlight(StopCommon):
    ...
    def HtmlRoundFlight(self, area, filename, list):
        ...
        for every in list:
            ...
            try:
                con_res = self.com_Hfunc(every, filename, area, 'share_flight_A')
                if con_res == 'error':
                    Dict[md5]['share_flight_A'] = round.share_flight_A
                else:
                    Dict[md5]['share_flight_A'] = con_res 
            except Exception, e:
                Dict[md5]['share_flight_A'] = round.share_flight_A
            
            try:
                con_res = self.com_Hfunc(every, filename, area, 'share_flight_B')
                if con_res == 'error':
                    Dict[md5]['share_flight_B'] = round.share_flight_B
                else:
                    Dict[md5]['share_flight_B'] = con_res 
            except Exception, e:
                Dict[md5]['share_flight_B'] = round.share_flight_B
            
            try:
                con_res = self.com_Hfunc(every, filename, area, 'stopby_A')
                if con_res == 'error':
                    Dict[md5]['stopby_A'] = round.stopby_A
                else:
                    Dict[md5]['stopby_A'] = con_res 
            except Exception, e:
                Dict[md5]['stopby_A'] = round.stopby_A
            
            try:
                con_res = self.com_Hfunc(every, filename, area, 'stopby_B')
                if con_res == 'error':
                    Dict[md5]['stopby_B'] = round.stopby_B
                else:
                    Dict[md5]['stopby_B'] = con_res 
            except Exception, e:
                Dict[md5]['stopby_B'] = round.stopby_B
